...
async function loadMetadataFromJson(pdfPath: string): Promise<Record<string, unknown>> {
...
async function optimizeImage(inputPath: string, outputPath: string): Promise<void> {
  await sharp(inputPath).jpeg({ quality: IMAGE_QUALITY }).toFile(outputPath);
}
...
function generateRSSFeed(slides: SlideInfo[]): string {
  ...
  sortedSlides.forEach((slide) => {
    ...
    const firstPageText =
      slide.pageTexts && slide.pageTexts.length > 0
...
async function main(): Promise<void> {
  ...
  console.log(
    `Converted ${slidesData.length} PDFs to images successfully in ${elapsedTime.toFixed(2)} seconds!`
  );
}
